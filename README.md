...
This is a specific Version for KNUT.

This branch can build on debian, when some dependencies reside in the parent directory.
...
```bash
git clone https://github.com/sigmike/avrdude.git 
cd avrdude
./bootstrap
./configure
make
...
```bash
git clone https://github.com/knutplot/Marlin.git
cd Marlin
make -C Marlin HARDWARE_MOTHERBOARD=43
```

Uploading should work like:

```bash
make -C Marlin HARDWARE_MOTHERBOARD=43 upload
```
